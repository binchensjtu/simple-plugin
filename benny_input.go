...
func (li *BennyInput) SetPipelineConfig(pConfig *p.PipelineConfig) {
	li.pConfig = pConfig
}

func (li *BennyInput) ConfigStruct() interface{} {
	baseDir := li.pConfig.Globals.BaseDir
	return &BennyInputConfig{
		RescanInterval:    "1m",
...
func (li *BennyInput) SetName(name string) {
	li.pluginName = name
}

func (li *BennyInput) Init(config interface{}) (err error) {
	var (
		errs    *ls.MultipleError
		oldest  time.Duration
		plugins []string
	)
	conf := config.(*BennyInputConfig)
...
func (li *BennyInput) startLogstreamInput(logstream *LogstreamInput, i int,
...
func (li *BennyInput) Run(ir p.InputRunner, h p.PluginHelper) (err error) {
	var (
		ok         bool
		errs       *ls.MultipleError
		newstreams []string
...
func (li *BennyInput) ReportMsg(msg *message.Message) error {
...
func init() {
	p.RegisterPlugin("BennyInput", func() interface{} {
		return new(BennyInput)
	})
}
